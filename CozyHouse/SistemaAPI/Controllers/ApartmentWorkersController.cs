...
namespace SistemaAPI.Controllers
{
    ...
    public class ApartmentWorkersController : ControllerBase
    {
        ...
        private readonly ILogger<ApartmentWorkersController> _logger;

        public ApartmentWorkersController(ContextDataBase context, IMapper mapper, ILogger<ApartmentWorkersController> logger)
        {
            _context = context;
            _mapper = mapper;
            _logger = logger;
        ...
        )
        {
            try
            {
                ...
                _logger.LogInformation("Consulta de apartamentos (trabajadores) realizada correctamente. Total: {Count}", totalCount);
                return Ok(new
                {
                    totalCount,
                    items = pagedResult
